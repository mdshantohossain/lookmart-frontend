...
import type { ProcessedVariantKeyType, ProductType } from "@/types";
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    setSelectedVariantKeys(first_variant_keys);
    setSelectedSize(first_variant_keys?.variant_key.split("-")[1] || "");
  ...
  useEffect(() => {
    ...
    if (selectedVariant.default) {
      setSelectedVariantKeys(selectedVariant.default);
      setSelectedSize(selectedVariant.default.variant_key.split("-")[1] || "");
      return;
    }
    ...
    const matched =
      variants.find((v) => v.variant_key === selectedVariantKeys.variant_key) ||
      variants[0];

    setSelectedVariantKeys(matched);
    setSelectedSize(matched.variant_key.split("-")[1] || "");
  }, [selectedVariant]);
  ...
  const handleAddToCart = (product: ProductType) => {
    if (!isExistedToCart) {
      console.log("Adding to cart:", {
        ...
        name: product.name,
        price: Number(selectedVariantKeys.selling_price) || product.selling_price,
        image: product.image_thumbnail,
        slug: product.slug,
        quantity: quantity,
        vid: selectedVariantKeys.vid || "",
        variant_id: selectedVariantKeys?.id,
        variant_key: selectedVariantKeys?.variant_key,
      ...
      //   name: product.name,
      //   price:
      //     Number(selectedVariantKeys.selling_price) || product.selling_price,
      //   image: product.image_thumbnail,
      //   slug: product.slug,
      //   quantity: quantity,
      //   vid: selectedVariantKeys.vid || "",
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setSelectedVariantKeys(variantKeys);
                              }}
